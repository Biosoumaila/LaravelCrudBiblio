             ...
             <div class="mb-3">
                ...
                <select class="form-select" id="author_id" name="author_id" required>
                    @foreach($authors as $author)
                        {{-- <option value="{{ $author->id }}" {{ $book->author_id == $author->id ? 'selected' : '' }}>$category->category_id --}}
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="category_id" name="category_id" required>
                    @foreach($categories as $category)
                        {{-- <option value="{{ $category->id }}" {{ $book->category_id == $category->id ? 'selected' : '' }}> $category->category_id --}}
